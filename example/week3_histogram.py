...
def main():
    ka_list = [0.0, 0.025, 0.05, 0.075, 0.1]  # length 5
    for ka in ka_list:
        submain(ka)
...
def submain(ka):
    ...
    for i_kn2, kn2 in enumerate(kn2_list):
        ...
        filename = dir1 + "kn{}ka{}_kn{}ka{}/".format(round(kn1, 4),
                                                      round(ka, 4),
                                                      round(kn2, 4),
                                                      round(ka, 4)) + "final_hst_list_k+{}k-{}_examples.csv".format(kp,
                                                                                                                    km)
    ...
    title = "test5_fig1__ka{}.pdf".format(ka)
    pp = PdfPages(title)
    pp.savefig(fig1)
    pp.close()
